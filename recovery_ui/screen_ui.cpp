...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    auto& logo = fastbootd_logo_enabled_ ? fastbootd_logo_ : clown_logo;
...
void ScreenRecoveryUI::draw_battery_capacity_locked() {
  int x;
  int y = margin_height_ + gr_get_height(clown_logo.get());
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
  ...
  clown_logo = LoadBitmap("logo_image");
...
int ScreenRecoveryUI::SelectMenu(const Point& p) {
  ...
  if (menu_) {
    if (!menu_->IsMain()) {
      // Back arrow hitbox
      const static int logo_width = gr_get_width(clown_logo.get());
      const static int logo_height = gr_get_height(clown_logo.get());
